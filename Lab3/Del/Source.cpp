...
enum MessageType { OBJECTCREATED, PLAYERJOINED, PLAYERLEFT, OBJECTSTATEUPDATE, ENDMESSAGE };
...
void Dot::writePacketData(PacketStream & streamIn) { 
	streamIn.writeInt(mPosX); 
	streamIn.writeInt(mPosY);
}

void Dot::receivePacketData(PacketStream & streamIn) { 
	streamIn.readInt(mPosX); 
	streamIn.readInt(mPosY); 
}
...
int main(int argc, char* args[])
{
	...
	p.writeInt(OBJECTCREATED); //write in object state  
	p.writeInt(OBJECTCREATED); //write in object state 
	p.writeInt(OBJECTSTATEUPDATE); //write in object state  
	p.writeInt(ENDMESSAGE);
